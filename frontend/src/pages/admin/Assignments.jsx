...
import { getAssignments, assignSubject, deleteAssignment, getStaff, getSubjects, getClasses } from '../../services/api';
...
export default function Assignments() {
  ...
  const [form, setForm] = useState({ staff_id: '', subject_id: '', class_id: '' });
  ...
  return (
    <Layout>
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => { setForm({ staff_id: '', subject_id: '', class_id: '' }); setModal(true); }} className="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700">+ Assign Subject</button>
      </div>
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <table className="w-full">
          <thead className="bg-gray-50"><tr>{['Staff', 'Subject', 'Class', 'Actions'].map(h => <th key={h} className="text-left px-4 py-3 text-sm font-semibold text-gray-600">{h}</th>)}</tr></thead>
          <tbody className="divide-y divide-gray-100">
            ...
            {assignments.length === 0 && <tr><td colSpan={4} className="px-4 py-8 text-center text-gray-400">No assignments</td></tr>}
          </tbody>
        </table>
      </div>
      <Modal isOpen={modal} onClose={() => setModal(false)} title="Assign Subject to Staff">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Subject (optional - leave blank for all subjects)</label><select value={form.subject_id} onChange={e => setForm({...form, subject_id: e.target.value})} className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500"><option value="">All Subjects</option>{subjects.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>

...
export default function AdminStaff() {
  ...
  const [form, setForm] = useState({ staff_id_number: '', first_name: '', last_name: '', department: '', password: '' });
  ...
  const openAdd = () => { setEditing(null); setForm({ staff_id_number: '', first_name: '', last_name: '', department: '', password: '' }); setIsOpen(true); };
  const openEdit = (s) => { setEditing(s); setForm({ staff_id_number: s.staff_id_number, first_name: s.first_name, last_name: s.last_name, department: s.department || '', password: '' }); setIsOpen(true); };

  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      if (editing) {
        await api.put(`/api/admin/staff/${editing.id}`, form);
    ...
    <AdminLayout>
      ...
      <div className="bg-white rounded-xl shadow p-4 mb-4">
        <div className="relative">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search staff..." className="w-full pl-10 pr-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" />
        </div>
      </div>
          ...
          <tbody className="divide-y divide-gray-100">
            {filtered.map(s => (
              <tr key={s.id} className="hover:bg-gray-50">
                <td className="px-6 py-4 font-mono">{s.staff_id_number}</td>
            ...
            {filtered.length === 0 && <tr><td colSpan={4} className="px-6 py-8 text-center text-gray-400">No staff found</td></tr>}
          </tbody>
        </table>
      </div>
      <Modal isOpen={isOpen} onClose={() => setIsOpen(false)} title={editing ? 'Edit Staff' : 'Add Staff'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          {!editing && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Staff ID Number *</label>
              <input value={form.staff_id_number} onChange={e => setForm({...form, staff_id_number: e.target.value})} className="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500" required />
            </div>
          )}
          ...
          <div>
            ...
            <input value={form.department} onChange={e => setForm({...form, department: e.target.value})} className="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
          </div>
          <div>

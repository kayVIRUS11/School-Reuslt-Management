...
import { getAllResults, getClasses, getSubjects, getTerms, getSessions } from '../../services/api';
...
export default function AllResults() {
  ...
  const [filters, setFilters] = useState({ class_id: '', subject_id: '', term_id: '', session_id: '', status: '' });
  ...
  useEffect(() => {
    ...
    getAllResults(params).then(r => setResults(r.data)).catch(() => {});
  }, [filters]);
  ...
  return (
    <Layout>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
        ...
        <select value={filters.term_id} onChange={e => setFilters({...filters, term_id: e.target.value})} className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"><option value="">All Terms</option>{terms.map(t => <option key={t.id} value={t.id}>{t.name} ({t.session_name})</option>)}</select>
        <select value={filters.session_id} onChange={e => setFilters({...filters, session_id: e.target.value})} className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"><option value="">All Sessions</option>{sessions.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select>
        <select value={filters.status} onChange={e => setFilters({...filters, status: e.target.value})} className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"><option value="">All Status</option>{['draft','submitted','approved','rejected'].map(s => <option key={s} value={s}>{s}</option>)}</select>
      </div>
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-x-auto">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-100">
            ...
            {results.length === 0 && <tr><td colSpan={11} className="px-4 py-8 text-center text-gray-400">No results found</td></tr>}
          </tbody>
        </table>
      </div>
    </Layout>

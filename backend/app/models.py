...
class Staff(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False)
    staff_id_number = db.Column(db.String(100), unique=True, nullable=False)
...
class StaffSubjectClass(db.Model):
    ...
    staff = db.relationship('Staff', backref='assignments', lazy=True)
    subject = db.relationship('Subject', backref='assignments', lazy=True)
    classroom = db.relationship('ClassRoom', backref='assignments', lazy=True)
    session = db.relationship('AcademicSession', backref='assignments', lazy=True)
...
class Result(db.Model):
    ...
    classroom = db.relationship('ClassRoom', backref='results', lazy=True)
    session = db.relationship('AcademicSession', backref='results', lazy=True)
    term = db.relationship('Term', backref='results', lazy=True)
    staff = db.relationship('Staff', backref='results', lazy=True)
    ...
    def to_dict(self):
        return {
            ...
            'remark': self.remark,
            'status': self.status,
            'student_name': f"{self.student.user.first_name} {self.student.user.last_name}" if self.student and self.student.user else None,
            'reg_number': self.student.reg_number if self.student else None,
            'subject_name': self.subject.name if self.subject else None,
            'class_name': self.classroom.name if self.classroom else None,
            'session_name': self.session.name if self.session else None,
